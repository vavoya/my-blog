...
export default function TableBody({folderId, pages, isError, hasNext, errorMessage, intersectionTarget}: TableBodyProps) {
    ...
    return (
        <ul key={'body'} role="rowgroup" className={styles.list}>
            {
                pages.map(paginatedPost => {
                    const openPostWindow: OpenPostWindow = () => {
                        const window = createWindowObj(
                            `PostWindow-${folderId}`,
                            buildFolderPath(folderObj[folderId], folderObj) + '/' + paginatedPost.post_name,
            ...
            <PostListFooter hasPage={pages.length > 0}
                            isError={isError}
                            hasNext={hasNext}
