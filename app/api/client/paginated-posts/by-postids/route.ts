...
import getPostsByPostIds from "@/models/post_info/getPostsByPostIds";
...
export async function GET(req: NextRequest): Promise<Response> {
    ...
    try {
        postIds = validateAndTransform(searchParams.getAll('postids'), 'string');
    } catch {
        return jsonResponse(response.badRequest('postids 파라미터가 유효하지 않습니다.'))
    }
