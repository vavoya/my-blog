...
import {createAuthIdQuery} from "@/app/api/server/banned-auth-list/createQuery";
...
export default async function getByAuthId(authId: UserInfoResponse['auth_id']) {
    const apiUrl = createAuthIdQuery(authId);
    ...
    try {
        const result = await fetch(apiUrl, {
            ...
            cache: "force-cache",
            next: { tags: ['all', authId, `${authId}-banned` ] }
        });
        const data: Response<BannedAuthListDocument | null> = await result.json();
        return data;
    } catch (error) {
